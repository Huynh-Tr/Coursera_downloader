...
def _download_binary_with_retry(
    url, destination, cookies, timeout_seconds=30, retry_count=1
):
    attempts = retry_count + 1
    for attempt in range(1, attempts + 1):
        try:
            response = requests.get(url, cookies=cookies, timeout=(10, timeout_seconds))
            ...
            logging.warning(
                "Timeout (%ss) while downloading %s (attempt %d/%d)",
                timeout_seconds,
                destination,
                attempt,
                attempts,
            ...
            if attempt >= attempts:
                logging.error("Skipping %s due to repeated timeout", destination)
                return False
        except requests.exceptions.RequestException as exc:
            logging.error("Failed to download %s: %s", destination, exc)
            return False
...
class QuizExamToMarkupConverter:
    ...
    def __call__(self, quiz_or_exam_json):
        ...
        for question_index, question_json in enumerate(quiz_or_exam_json["questions"]):
            ...
            if question_type not in self.KNOWN_QUESTION_TYPES:
                ...
                logging.info(
                    "Please report class name, quiz name and the data"
    ...
    def _convert_options(self, question_index, options, input_type):
        ...
        for option in options:
            ...
            result.append(
                '<label><input type="%s" name="%s">'
                "%s<br></label>" % (input_type, question_index, option_text)
            )
    ...
    def _generate_input_field(self):
        return [
            '<form><label>Enter answer here:<input type="text" '
            'name=""><br></label></form>'
        ]
...
class OnDemandCourseMaterialItemsV1:
    """
...
class Asset(namedtuple("Asset", "id name type_name url content_type data")):
    ...
    def __repr__(self):
        return (
            'Asset(id="%s", name="%s", type_name="%s", url="%s", content_type="%s", data="<...>")'
    ...
    def from_json(data):
        return ModulesV1(
            OrderedDict(
                (
                    item["id"],
                    ModuleV1(item["name"], item["id"], item["slug"], item["lessonIds"]),
                )
                for item in data
            )
        )
    ...
    def from_json(data):
        return LessonsV1(
            OrderedDict(
                (
                    item["id"],
                    LessonV1(item["name"], item["id"], item["slug"], item["itemIds"]),
                )
                for item in data
            )
        )
...
class VideosV1:
    ...
    def from_json(data):
        ...
        videos = [
            VideoV1(resolution, links["mp4VideoUrl"])
...
class CourseraOnDemand:
    ...
    def _get_notebook_folder(self, url, jupyterId, **kwargs):
        ...
        for content in reply["content"]:
            ...
            if content["type"] == "directory":
                ...
                head = "/".join(
                    [
                        clean_filename(dir, minimal_change=True)
                ...
                download_url = tmp_url.replace(" ", "%20")
                destination = self._course_name + "/notebook/" + head + "/" + tail
                if not os.path.exists(destination):
                    logging.info("Downloading %s into %s", tail, head)
                    _download_binary_with_retry(
                        download_url, destination, self._session.cookies
                    )
                else:
                ...
                supplement_links[str(extension[1:])].append((download_url, filename))
                ...
                supplement_links["ipynb"].append((download_url, filename))
    ...
    def _get_notebook_json(self, notebook_id, authorizationId):
        ...
        _ = newReq.get(
            OPENCOURSE_NOTEBOOK_TREE.format(
    ...
    def extract_links_from_lecture(
    ...
    def _extract_links_from_lecture_assets(self, asset_ids):
        ...
        def _add_asset(name, url, destination):
            ...
            extension = clean_filename(
                extension.lower().strip(".").strip(), self._unrestricted_filenames
            )
            basename = clean_filename(
                os.path.basename(filename), self._unrestricted_filenames
            )
            url = url.strip()
    ...
    def _get_asset_urls(self, asset_id):
        ...
        for element in dom["elements"]:
            ...
            if typeName == "asset":
                ...
                for asset in self._asset_retriever(
                    [open_course_asset_id], download=False
                ):
    ...
    def _extract_videos_and_subtitles_from_lecture(
        ...
        if resolution in videos:
            ...
            logging.debug(
                "Proceeding with download of resolution %s of <%s>.",
                resolution,
                video_id,
            )
        else:
    ...
    def _extract_subtitles_from_video_dom(self, video_dom, subtitle_language, video_id):
        ...
        for subtitle_node, subtitle_extension, subtitle_description in subtitle_nodes:
            ...
            if not download_all_subtitle and subtitle_set_nonexist:
                logging.warning(
                    "%s unavailable in '%s' language for video "
                    "with video id: [%s],"
                    "%s",
            ...
            if not subtitle_set_download:
                logging.warning(
                    "%s all requested subtitles are unavailable,"
    ...
    def extract_links_from_programming_immediate_instructions(self, element_id):
        ...
        """
        logging.debug(
            "Extracting links from programming immediate "
            "instructions for element_id <%s>.",
            element_id,
        )
        ...
        try:
            ...
            instructions = (
                IN_MEMORY_MARKER + self._markup_to_html(text),
                "instructions",
            )
            extend_supplement_links(
            ...
            logging.error(
                "Could not download programming assignment %s: %s",
                element_id,
                exception,
            )
            if is_debug_run():
    ...
    def extract_links_from_peer_assignment(self, element_id):
        ...
        try:
            ...
            logging.error(
                "Could not download peer assignment %s: %s", element_id, exception
            )
            if is_debug_run():
                logging.exception(
                    "Could not download peer assignment %s: %s", element_id, exception
    ...
    def extract_links_from_supplement(self, element_id):
        ...
        try:
            ...
            for asset in dom["linked"]["openCourseAssets.v1"]:
                ...
                instructions = (
                    IN_MEMORY_MARKER + self._markup_to_html(value),
                    "instructions",
                )
                extend_supplement_links(
            ...
            if is_debug_run():
                logging.exception(
                    "Could not download supplement %s: %s", element_id, exception
                )
            return None
    ...
    def _extract_asset_urls(self, asset_ids):
        ...
        return [
            {"id": element["id"], "url": element["url"].strip()}
            for element in dom["elements"]
        ]
    ...
    def extract_references_poll(self):
        try:
            dom = get_page(
                self._session,
                OPENCOURSE_REFERENCES_POLL_URL.format(course_id=self._course_id),
                json=True,
            )
    ...
    def extract_links_from_reference(self, short_id):
        ...
        try:
            ...
            if is_debug_run():
                logging.exception(
                    "Could not download supplement %s: %s", short_id, exception
                )
            return None
    ...
    def _extract_programming_immediate_instructions_text(self, element_id):
        ...
        return [
            element["assignmentInstructions"]["definition"]["value"]
            for element in dom["elements"]
        ]
    ...
    def _extract_assignment_text(self, element_id):
        ...
        return [
            element["submissionLearnerSchema"]["definition"]["assignmentInstructions"][
                "definition"
            ]["value"]
            for element in dom["elements"]
        ]
    ...
    def _extract_peer_assignment_text(self, element_id):
        ...
        for element in dom["elements"]:
            ...
            if "introduction" in element["instructions"]:
                result.append(
                    element["instructions"]["introduction"]["definition"]["value"]
                )
    ...
    def _extract_links_from_text(self, text):
        ...
        extend_supplement_links(
            supplement_links, self._extract_links_from_asset_tags_in_text(text)
        )
    ...
    def _extract_links_from_asset_tags_in_text(self, text):
        ...
        for asset in asset_urls:
            title = clean_filename(
                asset_tags_map[asset["id"]]["name"], self._unrestricted_filenames
            )
            extension = clean_filename(
    ...
    def _extract_links_from_a_tags_in_text(self, text):
        ...
        soup = BeautifulSoup(text)
        links = [
            item["href"].strip() for item in soup.find_all("a") if "href" in item.attrs
        ]
