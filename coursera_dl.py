...
def create_session(args):
    ...
    if args.cookies_cauth:
        ...
        try:
            from cookies import get_edge_cookies_for_coursera
            ...
            logging.error(f'Failed to extract Edge cookies: {e}')
            # Fall back to manual login
            login(session, args.username, args.password)
    elif args.cookies_file:
        # Cookies file provided - load from file, no login needed
        from cookies import find_cookies_for_class
        # We don't have class_name yet at session creation, so just load all cookies
        from cookies import get_cookie_jar
        cj = get_cookie_jar(args.cookies_file)
        for cookie in cj:
            session.cookies.set_cookie(cookie)
        logging.info('Loaded cookies from file: %s', args.cookies_file)
    else:
        login(session, args.username, args.password)
    return session
