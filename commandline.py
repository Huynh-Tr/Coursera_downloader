...
def class_name_arg_required(args):
    ...
    """
    no_class_name_flags = ['list_courses', 'version', 'save_edge_cookies']
    return not any(
        getattr(args, flag)
...
def parse_args(args=None):
    ...
    group_adv_auth.add_argument(
        '--edge-cookies',
        dest='use_edge_cookies',
        action='store_true',
        default=False,
        help='extract cookies from Microsoft Edge browser automatically')

    group_adv_auth.add_argument(
        '--save-edge-cookies',
        dest='save_edge_cookies',
        action='store',
        default=None,
        help='extract cookies from Edge and save to specified file')
    ...
    # Handle Edge cookies options
    if args.save_edge_cookies:
        try:
            from cookies import save_edge_cookies_to_file
            saved_path = save_edge_cookies_to_file(args.save_edge_cookies)
            if saved_path:
                print(f"Edge cookies saved to: {saved_path}")
            else:
                logging.error("Failed to save Edge cookies")
                sys.exit(1)
    ...
    if not args.cookies_file and not args.cookies_cauth and not args.browser and not args.use_edge_cookies:
        try:
