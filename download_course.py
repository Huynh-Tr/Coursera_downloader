...
def extract_course_slug(course_url):
    ...
    if match:
        ...
        raise ValueError(
            f"Invalid Coursera URL: {course_url}. Expected format: https://www.coursera.org/learn/course-name"
        )
...
def download_coursera_course(course_url, output_path=None, cookies_file=None):
    ...
    try:
        ...
        if not os.path.exists(cookies_file):
            ...
            logging.error(
                "Please ensure 'coursera_cookies.txt' exists in the current directory."
            )
            logging.error(
                "You can create it by running: python coursera_dl.py --save-edge-cookies coursera_cookies.txt"
            )
            return False
        ...
        try:
            ...
            logging.error(
                "Please ensure coursera_dl.py and commandline.py are in the same directory."
            )
            return False
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            '  python download_course.py "https://www.coursera.org/learn/ml-intro" "./Downloads" "./my_cookies.txt"'
        )
        print("\nArguments:")
        print("  course_url    : Full Coursera course URL (required)")
        print(
            "  output_path   : Directory to save files (optional, default: ./Downloads)"
        )
        print(
            "  cookies_file  : Path to cookies file (optional, default: ./coursera_cookies.txt)"
        )
        print("\nNote:")
        print("  Make sure you have a valid cookies file before running this script.")
        print(
            "  You can create one using: python coursera_dl.py --save-edge-cookies coursera_cookies.txt"
        )
        print("=" * 80)
        sys.exit(1)
